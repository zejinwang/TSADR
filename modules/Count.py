...
class Easy_Deformable(nn.Module):
    def __init__(self, nf=64, groups=8):
        super(Easy_Deformable, self).__init__()
...
class LunaTokis(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx in range(N-1):
            ...
            fusion_fea = self.fusion(aligned_fea) # [B, C, H, W]
            if idx == 0:
                to_lstm_fea.append(fea1[0])
            to_lstm_fea.append(fusion_fea)
            to_lstm_fea.append(fea2[0])
        lstm_feats = torch.stack(to_lstm_fea, dim = 1) # [B, N, C, H, W]
...
class RSABlock(nn.Module):
    def __init__(self, nf=64, groups=8):
        ...
        # fea1
        self.offset_conv1 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)  # concat for diff
        self.offset_conv2 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
    ...
    def forward(self, x):
        ...
        align = self.lrelu(self.dcnpack(x, offset))
        out = self.conv(align) + x

        return out


class DCB_Layer(nn.Module):
    def __init__(self, nf, groups):
        super(DCB_Layer, self).__init__()

        self.easy_align1 = Easy_Deformable(nf, groups)
        self.easy_align2 = Easy_Deformable(nf, groups)

    def forward(self, x):
        f0, f1, f2 = x 
        ali_fea1 = self.easy_align1(f0, f1)
        ali_fea3 = self.easy_align2(f2, f1)
        ali_fea2 = f1

        return [ali_fea1, ali_fea2, ali_fea3]


class PCDNet(nn.Module):
    def __init__(self, nf=64, groups=8, front_RBs=5, num_DCBs=3, back_RBs=10):
        ...
        rsa = [RSABlock(nf, groups) for _ in range(3)]
        self.rsa = nn.Sequential(*rsa)

        self.fusion1 = nn.Conv2d(2 * nf, nf, 1, 1, bias=True)

        #### deformation correction
        corrector = [DCB_Layer(nf, groups) for _ in range(num_DCBs)]
        self.corrector = nn.Sequential(*corrector)

        self.fusion2 = nn.Sequential(*[
            nn.Conv2d(3 * nf, nf, 1, 1, bias=True),
    ...
    def forward(self, frame0, frame2):
        x = torch.stack((frame0,frame2),dim=1)  # 2 input video frames
        B, N, C, H, W = x.size()
        ...
        for idx in range(N-1):
            ...
            fusion_fea = self.fusion1(aligned_fea) # [B, C, H, W]
            fusion_fea = self.rsa(fusion_fea)
            if idx == 0:
                to_lstm_fea.append(fea1[0])
            to_lstm_fea.append(fusion_fea)
            to_lstm_fea.append(fea2[0])

            feat1, feat2, feat3 = self.corrector(to_lstm_fea)
            #_, feats, _ = self.corrector(to_lstm_fea)

        feats = self.fusion2(torch.cat((feat1, feat2, feat3), dim=1))
